...
# Utility function to filter syllabus data for LLM
def filter_syllabus_data(syllabus: List[Dict]) -> List[Dict]:
    filtered_syllabus = []
    for unit in syllabus:
        filtered_unit = {
            "unit_number": unit["unit_number"],
            "title": unit["title"],
            "topics": [topic["topic_name"] for topic in unit["topics"]]
        }
        filtered_syllabus.append(filtered_unit)
    return filtered_syllabus

# Endpoint to process syllabus with LLM
@app.post("/courses/{course_id}/process-syllabus")
def process_syllabus_with_llm(course_id: str):
    # Get the full syllabus
    syllabus = get_full_syllabus(course_id)

    # Filter the syllabus data
    filtered_syllabus = filter_syllabus_data(syllabus)

    # Prepare the LLM input
    llm_input = {
        "prompt": f"Process the following syllabus:\n{filtered_syllabus}",
        "max_tokens": 500,
        "temperature": 0.7
    }
